...
const Sidebar = () => {
  ...
  const links: Path[] = [
    { path: "/administrator-portal", title: "home", Icon: FaHome },
    { path: "#", title: "settings", Icon: FaGear },
  ];

  return (
    <aside
      className={`transition-width relative hidden h-full rounded-md bg-gray-950 duration-300 sm:block ${
        isExpand ? "w-64" : "w-20"
      }`}
    >
      <Button
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Link
                    key={index}
                    href={path}
                    className={`flex w-full transform items-center rounded-md px-3.5 py-2 text-gray-300 transition-colors duration-300 hover:bg-gray-800 hover:text-gray-200 ${
                      pathname.startsWith(path) ? "bg-gray-800" : ""
                    }`}
                  >
                    <div
